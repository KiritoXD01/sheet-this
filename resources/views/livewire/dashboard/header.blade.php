    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-20">
            <div class="flex items-center gap-2">
                ...
                <span class="font-bold text-xl tracking-tight text-slate-900 dark:text-slate-100">Sheet This</span>
            </div>
            <div class="hidden md:flex items-center space-x-8">
                @foreach ($navLinks as $link)
                    ...
                    @else
                        <a class="text-sm font-medium text-slate-600 dark:text-slate-400 hover:text-primary transition-colors py-1"
                            href="{{ $link['route'] }}">{{ $link['title'] }}</a>
                    @endif
                @endforeach
            </div>
